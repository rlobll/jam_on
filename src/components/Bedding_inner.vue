          ...
          <div>
            <div class="buy_btn_wrap">
              ...
              <button type="button" class="heart" @click="addToHeart">
                <img src="../assets/jam_on_heart.png" alt="" />{{ heartCount }}
              </button>
            </div>
            <div class="pay_btn">
              ...
              <button type="button" class="save">찜</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    removeProduct() {
      // 제품 삭제 로직 구현
      this.selectedProduct = ""; // "제품(필수)"를 선택한 것과 동일한 효과
    },
    decreaseQuantity() {
    ...
    increaseQuantity() {
      ...
      if (this.selectedProduct) {
        ...
        if (!this.selectedProduct.quantity) {
          // 수량이 없으면
          this.selectedProduct.quantity = 1; // 수량을 1로 설정
        } else {
          this.selectedProduct.quantity++; // 수량 증가
        }
      }
